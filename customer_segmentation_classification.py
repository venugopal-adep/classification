...
def train_initial_model(X_train, y_train, initial_params):
    model = DecisionTreeClassifier(**initial_params)
    model.fit(X_train, y_train)
    return model

def perform_gridsearch(model, X_train, y_train, param_grid):
...
def main():
    ...
    st.title("GridSearch Optimizer: Decision Tree on Customer Segmentation Data")
    ...
    column_descriptions = {
        ...
        "Graduated": "Educational qualification of the customer",
        "Profession": "Profession of the customer",
        "Work_Experience": "Work experience of the customer",
    ...
    if show_bivariate:
        ...
        fig = px.scatter(df, x=x_feature, y=y_feature, color='Segmentation', title=f"{x_feature} vs {y_feature}")
        st.plotly_chart(fig, use_container_width=True)
    ...
    if show_initial:
        ...
        st.json({'criterion': 'gini', 'splitter': 'best', 'random_state': 42})
    ...
    if show_gridsearch:
        ...
        st.json(param_grid)
        grid_search = perform_gridsearch(model, X_train, y_train, param_grid)
    else:
    ...
    if show_comparison:
        ...
        param_comparison = pd.DataFrame({
            ...
            'Initial Value': ['gini', 'best', 'None', 'N/A'],  # Corresponding to the initial parameters
