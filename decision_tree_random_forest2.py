...
st.write("- Random forests introduce randomness by using a random subset of features for each tree.")
st.write("- They are more robust and accurate than individual decision trees but less interpretable.")

st.write("In this example, a single decision tree is compared with a random forest classifier. The decision tree is trained without using bagging, while the random forest classifier uses an ensemble of decision trees.")
