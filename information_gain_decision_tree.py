...
st.write("Information gain is a measure used in decision trees to determine the best split at each node. It quantifies the reduction in impurity achieved by splitting the data based on a particular feature.")
st.latex(r"IG(t, a) = Gini(t) - \sum_{v \in Values(a)} \frac{|t_v|}{|t|} Gini(t_v)")
st.write("Where:")
...
st.write("In the decision tree plot above, each node shows the feature and threshold used for splitting, along with the information gain at that node. The colors of the nodes represent the majority class at each node.")
...
st.write("Information gain is a key concept in decision tree learning, allowing the algorithm to determine the best splits based on the reduction in impurity. By maximizing the information gain at each split, the decision tree aims to create pure leaf nodes, leading to accurate classifications.")
